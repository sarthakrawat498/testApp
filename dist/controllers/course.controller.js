...
exports.update = exports.create = exports.getById = exports.getByPathway = exports.getAll = void 0;
const repository = __importStar(require("../repositories/course.repository"));
...
const getByPathway = async (request, response, next) => {
    try {
        response.json(await repository.findByPathway(Number(request.params.pathwayId)));
    }
    catch (error) {
        next(error);
    }
};
exports.getByPathway = getByPathway;
...
const create = async (request, response, next) => {
    const parsed = course_validator_1.createCourseSchema.safeParse(request.body);
    if (!parsed.success)
...
const update = async (request, response, next) => {
    const parsed = course_validator_1.updateCourseSchema.safeParse(request.body);
    if (!parsed.success)
        return response.status(400).json({ errors: parsed.error.errors });
    try {
